...
char *arg;
{
	...
	be_user();
	rundir = getcwd((char *)malloc(512), 512);
	if (!rundir) {
		perror("getcwd");
		exit(1);
	}
	...
	*class++ = '\0';
	...
	if (access("/usr/bin/compress", X_OK) == 0) {
		compresscmd = "/usr/bin/compress";
	} else if (access("/usr/bin/gzip", X_OK) == 0) {
...
maketar()
{
	...
	*tvp = 0;
	...
	sprintf(assignment_file, "#%s-%05d-%s", user_name, getpid(), myname);
	tempfile = strdup(assignment_path);
...
#ifdef DUMPMEM
void dumpmem(void *p, int a, int b) {
	int o;
	for (o=a; o<=b; o+=4)
		printf("0x%08x+%03d: 0x%08x\n",p,o,*((int*)(p+o)));
	}
#define DUMPMEM(p,a,b) dumpmem(p,a,b)
#else
#define DUMPMEM(p,a,b) {}
#endif
